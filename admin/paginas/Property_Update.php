...
$formData['valor'] = 'R$ ' . number_format($formData['valor'], 2, ',', '.');
...
// If property has state and city, get lists for those
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Preprocess the data
    $updatedData['valor'] = str_replace(['R$', '.', ','], ['', '', '.'], $updatedData['valor']);
    
    // Validate form data
    if (empty($updatedData['titulo'])) {
        $error = 'O título do imóvel é obrigatório.';
    } elseif (empty($updatedData['codigo'])) {
        $error = 'O código do imóvel é obrigatório.';
    } elseif (empty($updatedData['id_categoria'])) {
        $error = 'A categoria do imóvel é obrigatória.';
    } elseif (empty($updatedData['valor'])) {
        $error = 'O valor do imóvel é obrigatório.';
    } else {
        // Update property using function from admin_functions.php
        $result = updateProperty($property_id, $updatedData);
        ...
        if ($result) {
            ...
            for ($i = 1; $i <= 12; $i++) {
                ...
                if ($uploadFile && $uploadFile['error'] === 0) {
                    ...
                    $fileName = $updatedData['codigo'] . $imageNumber . '.jpg';
            ...
            $error = 'Ocorreu um erro ao atualizar o imóvel. Verifique se o código já não está sendo utilizado por outro imóvel.';
        }
    }
    
    // If there's an error, get city and neighborhood lists for form repopulation
    if (!empty($updatedData['id_estado'])) {
        $cidades = getCitiesByState($updatedData['id_estado']);
    }
    
    if (!empty($updatedData['id_cidade'])) {
        $bairros = getNeighborhoodsByCity($updatedData['id_cidade']);
    }
    
    // Update formData with POST values for form re-population in case of error
    $formData = $updatedData;
    // Re-format the price for display
    $formData['valor'] = 'R$ ' . number_format($formData['valor'], 2, ',', '.');
}
?>
    ...
    <form method="POST" enctype="multipart/form-data" class="admin-form">
        ...
        <div class="form-section">
            ...
            <div class="current-images">
                ...
                <p class="form-text">Para substituir uma imagem, faça o upload da nova.</p>
                ...
                <div class="image-gallery">
                    ...
                    <div class="image-preview">
                        <img src="<?= $uploadDir . $fileName . '?v=' . time() ?>" alt="Imagem <?= $i ?>">
                        <div class="image-number">Imagem <?= $i ?></div>
                    </div>
                    <?php endif; endfor; ?>
                </div>
            </div>
